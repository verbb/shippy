<?php
namespace verbb\shippy\rates\royalmail\postoffice;

use verbb\shippy\carriers\RoyalMailRates;

class InternationalTracked extends RoyalMailRates
{
    // Static Methods
    // =========================================================================

    public static function getRates(string $countryCode): array
    {
        $zone = self::getZone($countryCode);

        if ($zone === 'UK') {
            return [];
        }

        $included = ['AX', 'AD', 'AU', 'AT', 'BE', 'BR', 'CA', 'HR', 'CY', 'DK', 'EE', 'FO', 'FI', 'FR', 'DE', 'GI', 'GR', 'GL', 'HK', 'HU', 'IS', 'IN', 'IE', 'IL', 'IT', 'LV', 'LB', 'LI', 'LT', 'LU', 'MY', 'MT', 'NL', 'NZ', 'NO', 'PL', 'PT', 'RU', 'SM', 'RS', 'SG', 'SK', 'SI', 'KR', 'ES', 'SE', 'CH', 'TR', 'US', 'VA'];

        if (!in_array($countryCode, $included)) {
            return [];
        }

        $bands = [
            '2024' => [
                self::LETTER => [
                    100 => [790, 790, 790, 790, 790, 790],
                ],
                self::LARGE_LETTER => [
                    100 => [950, 950, 950, 1055, 1060, 1060],
                    250 => [1050, 1050, 1050, 1215, 1335, 1240],
                    500 => [1160, 1160, 1160, 1420, 1615, 1450],
                    750 => [1205, 1205, 1205, 1625, 1925, 1670],
                ],
                self::PACKET => [
                    250 => [1205, 1240, 1325, 1525, 1670, 1720],
                    500 => [1335, 1370, 1500, 1970, 2180, 2185],
                    750 => [1435, 1475, 1595, 2240, 2485, 2470],
                    1000 => [1500, 1545, 1670, 2530, 2810, 2765],
                    1250 => [1550, 1590, 1755, 2745, 3120, 3185],
                    1500 => [1550, 1590, 1845, 2900, 3395, 3185],
                    2000 => [1550, 1745, 1920, 3020, 3580, 3185],
                ],
                self::PRINTED_PAPERS => [
                    250 => [1205, 1240, 1325, 1525, 1670, 1720],
                    500 => [1335, 1370, 1500, 1970, 2180, 2185],
                    750 => [1435, 1475, 1595, 2240, 2485, 2470],
                    1000 => [1500, 1545, 1670, 2530, 2810, 2765],
                    1250 => [1550, 1590, 1755, 2745, 3120, 3185],
                    1500 => [1550, 1590, 1845, 2900, 3395, 3185],
                    2000 => [1550, 1745, 1920, 3020, 3580, 3185],
                ],
            ],
            '2025' => [
                self::LETTER => [
                    100 => [810, 810, 810, 810, 810, 810],
                ],
                self::LARGE_LETTER => [
                    100 => [975, 975, 975, 1080, 1085, 1070],
                    250 => [1075, 1075, 1075, 1245, 1370, 1250],
                    500 => [1190, 1190, 1190, 1455, 1655, 1465],
                    750 => [1235, 1235, 1235, 1665, 1975, 1690],
                ],
                self::PACKET => [
                    250 => [1240, 1270, 1390, 1600, 1755, 1685],
                    500 => [1370, 1405, 1575, 2070, 2290, 2185],
                    750 => [1425, 1510, 1675, 2350, 2610, 2495],
                    1000 => [1500, 1585, 1755, 2655, 2950, 2805],
                    1250 => [1550, 1630, 1845, 2880, 3275, 3235],
                    1500 => [1550, 1630, 1935, 3045, 3565, 3235],
                    2000 => [1550, 1790, 2015, 3170, 3760, 3235],
                ],
            ],
            '2025-10' => [

                self::LETTER => [
                    100 => [810, 810, 810, 810, 810, 810],
                ],
                self::LARGE_LETTER => [
                    100 => [975, 975, 975, 1080, 1085, 1070],
                    250 => [1075, 1075, 1075, 1245, 1370, 1250],
                    500 => [1190, 1190, 1190, 1455, 1655, 1465],
                    750 => [1235, 1235, 1235, 1665, 1975, 1690],
                ],
                self::PACKET => [
                    250 => [1240, 1270, 1390, 1600, 1755, 1685],
                    500 => [1370, 1405, 1575, 2070, 2290, 2185],
                    750 => [1425, 1510, 1675, 2350, 2610, 2495],
                    1000 => [1500, 1585, 1755, 2655, 2950, 2805],
                    1250 => [1550, 1630, 1845, 2880, 3275, 3235],
                    1500 => [1550, 1630, 1935, 3045, 3565, 3235],
                    2000 => [1550, 1790, 2015, 3170, 3760, 3235],
                ],
            ],
            '2026' => [
                self::LETTER => [
                    100 => [810, 810, 810, 810, 810, 810],
                ],
                self::LARGE_LETTER => [
                    100 => [995, 995, 995, 1190, 1195, 1125],
                    250 => [1075, 1075, 1075, 1355, 1490, 1385],
                    500 => [1215, 1215, 1215, 1555, 1655, 1540],
                    750 => [1240, 1240, 1240, 2025, 1960, 1920],
                ],
                self::PACKET => [
                    250 => [1250, 1310, 1670, 1990, 1755, 1785],
                    500 => [1330, 1445, 1850, 2590, 2290, 2315],
                    750 => [1440, 1555, 1875, 2700, 2610, 2595],
                    1000 => [1515, 1635, 1915, 3055, 2950, 2955],
                    1250 => [1565, 1690, 1940, 3370, 3295, 3365],
                    1500 => [1565, 1690, 1940, 3590, 3565, 3365],
                    2000 => [1635, 1845, 2095, 3690, 3760, 3365],
                ],
            ],
        ];

        return self::getInternationalBoxPricing($bands, $countryCode);
    }
}
